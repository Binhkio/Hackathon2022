import { ref, child, onValue, update, remove, set, push, get } from "firebase/database"
...
export const Game = () => {
    ...
    useEffect(()=>{
        ...
        if(userIdArray.indexOf(userID) === gameForm.cur_turn % userIdArray.length){
            // Answer question
            setYourTurn(true)
            setTimeout(()=>{
                if(!answered){
            ...
            if(answered && !correctAns){
                const lastState = gameForm.turns ? gameForm.turns[`${gameForm.cur_turn}`].state :  {
                    "question_id": `${Math.floor(Math.random() * 110)}`,
                    "state": {
                        [userID]: {
                            "hp": 100,
    ...
    return (
        ...
        <div className="line">
            {gameForm && Object.keys(gameForm.users?gameForm.users:{}).map((key, idx) => (
                <div className="player" key={idx}>
                    <div className="name">Player 1</div>
                    <div className="hp">{turnForm?.state && turnForm?.state[key]?.hp} HP</div>
                </div>
            ))}
            
        </div>
        <div>
            <button disabled={started} onClick={handleStartGame}>Start</button>
        </div>
        {yourTurn &&
            <div className="question">
                <div className="content">{questions && questions.content}</div>
                <div className="answers">
                    <div id="ans-1" onClick={handleAnswer('1')} >A. {questions?.answer && questions.answers[1]}</div>
                    <div id="ans-2" onClick={handleAnswer('2')} >B. {questions?.answer && questions.answers[2]}</div>
                    <div id="ans-3" onClick={handleAnswer('3')} >C. {questions?.answer && questions.answers[3]}</div>
                    <div id="ans-4" onClick={handleAnswer('4')} >D. {questions?.answer && questions.answers[4]}</div>
                </div>
            </div>
        }
        <div className="line">
            <div className="user">
                ...
                <div className="hp">{turnForm?.state && turnForm.state[`${userID}`].hp} HP</div>
            </div>
        </div>
        </>
    )
